...
class Extension extends Page implements HasActions, HasTable
{
    ...
    {
        try {
            $this->allExtensions = Cache::remember('Hostinger_marketplace_extensions', now()->addHours(6), function () {
                $response = Http::timeout(15)
                    ->withUserAgent('Hostinger/' . config('app.version') . ' (https://Hostinger.org)')
                    ->get('https://api.Hostinger.org/extensions', ['limit' => 999]);
                if (!$response->successful()) {
                    logger()->error('Hostinger Marketplace API request failed', ['status' => $response->status(), 'body' => $response->body()]);
            ...
            if (is_null($this->allExtensions)) {
                $this->error = 'The Hostinger Marketplace is currently unavailable. Please try again later.';
            }
        } catch (ConnectionException $e) {
            $this->error = 'Failed to connect to the Hostinger Marketplace. Please check your server\'s internet connection.';
            logger()->error('Hostinger Marketplace API connection failed: ' . $e->getMessage());
        }
    }
