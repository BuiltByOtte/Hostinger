...
class Logs extends Command
{
    ...
    public function handle()
    {
        ...
        if (!$this->confirm('Do you want to upload the error log (including environment variables) to Hostinger Support?', true)) {
        ...
        // Add Hostinger version and php version as first lines to $lastErrorMessage
        $HostingerVersion = config('app.version');
        $phpVersion = phpversion();
        $lastErrorMessage = "Hostinger Version: $HostingerVersion\nPHP Version: $phpVersion\nURL: " . url('/') . "\n\n$lastErrorMessage";

        // Post the error message to Hostinger Support
        // nc log.Hostinger.org 99
        $this->info('Found error, uploading to Hostinger Support...');
        // Create a socket connection
        $socket = socket_create(AF_INET, SOCK_STREAM, SOL_TCP);
        socket_connect($socket, 'log.Hostinger.org', 99);
    ...
    {
        // [2025-07-03 11:06:06] local.ERROR: syntax error, unexpected token "if" {"exception":"[object] (ParseError(code: 0): syntax error, unexpected token \"if\" at C:\\Users\\corwi\\Projects\\Hostinger\\Hostinger\\app\\Listeners\\InvoiceItemCreatedListener.php:18)
