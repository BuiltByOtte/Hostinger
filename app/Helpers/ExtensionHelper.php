...
class ExtensionHelper
{
    ...
    {
        $extension = '\\Hostinger\\Extensions\\' . ucfirst($type) . 's\\' . $extension . '\\' . $extension;
    ...
    public static function getAvailableExtensions()
    {
        ...
        foreach ($classmap as $class => $path) {
            if (strpos($class, 'Hostinger\\Extensions\\') !== 0) {
                continue;
            }

            // Example: Hostinger\Extensions\Whatevers\SomeExtension\SomeExtension
            $parts = explode('\\', $class);

            // Must have at least: Hostinger, Extensions, <Type>s, <Name>, <Class>
            if (count($parts) < 5) {
                continue;
            }
        ...
        foreach ($typeFolders as $typeFolder) {
            ...
            foreach ($extensionDirs as $extensionDir) {
                ...
                if (class_exists('\\Hostinger\\Extensions\\' . ucfirst($type) . 's\\' . $name . '\\' . $name)) {
                    $reflection = new ReflectionClass('\\Hostinger\\Extensions\\' . ucfirst($type) . 's\\' . $name . '\\' . $name);
                    $attributes = $reflection->getAttributes(ExtensionMeta::class);
